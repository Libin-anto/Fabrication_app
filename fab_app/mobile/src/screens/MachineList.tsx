...
export default function MachineList({ navigation }: any) {
  ...
  const renderMachine = ({ item }: { item: any }) => {
    const isRepair = item.status === 'Under Repair';
    return (
      <View style={styles.card}>
        {confirmDeleteId === item.id ? (
          ...
          <TouchableOpacity 
            ...
            onPress={() => navigation.navigate('MachineForm', { machine: item })}
          >
            <View style={styles.cardIconContainer}>
              <Feather name="tool" size={20} color="#3A322B" />
            </View>
            <View style={styles.cardDetailsMain}>
              <View style={styles.cardHeader}>
                <Text style={styles.machineName}>{item.name}</Text>
                <View style={styles.statusBadge}>
                  <View style={[styles.statusDot, isRepair && styles.statusDotRepair]} />
                  <Text style={[styles.statusBadgeText, isRepair && styles.statusBadgeTextRepair]}>
                    {item.status.charAt(0).toUpperCase() + item.status.slice(1)}
                  </Text>
                </View>
              </View>
              <View style={styles.cardDetails}>
                <Text style={styles.detailText}>ID: {item.machine_id}</Text>
                <Text style={styles.detailText}>Cat: {item.category}</Text>
              </View>
            </View>
          </TouchableOpacity>
        )}
