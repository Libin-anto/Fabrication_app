...
if status in (200, 201) and w_res:
    ...
    log_req_res("DELETE", f"{BASE_URL}/workers/{w_id}")
    log_req_res("GET", f"{BASE_URL}/workers/")
...
if status in (200, 201) and m_res:
    ...
    log_req_res("DELETE", f"{BASE_URL}/machines/{m_id}")
    log_req_res("GET", f"{BASE_URL}/machines/")
...
if w_assign_res and m_assign_res:
    ...
    status, a_res = log_req_res("POST", f"{BASE_URL}/assignments/assign", {"worker_id": w_id2, "machine_id": m_id2, "location": "On Site"})
    ...
    if status in (200, 201) and a_res:
        a_id = a_res.get("id")
...
log_req_res("GET", f"{BASE_URL}/workers/")
log_req_res("GET", f"{BASE_URL}/machines/?status=Available")
log_req_res("GET", f"{BASE_URL}/machines/?status=Assigned")
import urllib.parse
log_req_res("GET", f"{BASE_URL}/machines/?status={urllib.parse.quote('Under Repair')}")

# 5. Search
log_lines.append("\n=== SEARCH ===")
log_req_res("GET", f"{BASE_URL}/search/?q=Assign")
log_req_res("GET", f"{BASE_URL}/search/?q=AM-001")
log_req_res("GET", f"{BASE_URL}/search/?q=xyz123nonsense")
